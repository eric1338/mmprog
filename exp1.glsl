...
float getStarDistance(vec3 rayPoint) {
	...
	if (rayPoint.z < sV) return sV - rayPoint.z + 0.1;
	
	if (rayPoint.z > 40) return 999;
	
	float xVal = floor(rayPoint.x / 2);
...
float getPlanetDistance(vec3 rayPoint) {
	...
	float p4Dist = getSphereDistance(vec3(0, -110, -10), 100, rayPoint);
	
	return min(min(min(p1Dist, p2Dist), p3Dist), p4Dist);
}
...
vec3 getBackgroundColor(vec3 hitPoint) {
	...
	//vec3 bgColor = vec3(0.0, 0.2 + g, 0.9) * 0.1;
	
	float scale = 60;
	
	float myY = (hitPoint.y + (scale / 2)) / scale;
	
	float b = 1 - max(min(1, myY), 0);
	
	return vec3(0, b * 0.5, b) * 0.4;
}
